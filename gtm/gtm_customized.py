...
from gtm.autoencoders_task1 import AutoEncoderMLP
from gtm.autoencoders_task3 import AutoEncoderMLP as AutoEncoderMLP2
from gtm.autoencoders_task4 import AutoEncoderMLP as AutoEncoderMLP3
from gtm.predictors import Predictor, PredictorMLP
...
class GTM:
  ...
  def __init__(
      ...
      separate_decoders=False, # whether to use separate decoders
    ...
    if train_data.M_labels is not None:
      if len(train_data.M_labels.shape) == 1:
        train_data.M_labels = train_data.M_labels.reshape(-1, 1)
      labels_size = train_data.M_labels.shape[1]
      if predictor_type == 'classifier':
        # n_labels = len(np.unique(train_data.M_labels))
        n_labels = np.max(train_data.M_labels) + 1
      else:
        n_labels = 1
    else:
      labels_size = 0
    ...
    predictor_dims.extend([2])
    self.language_predictor = Predictor(
      predictor_dims=predictor_dims,
      predictor_non_linear_activation=predictor_non_linear_activation,
      predictor_bias=predictor_bias,
      dropout=dropout
    ...
    self.log_file.write('AutoEncoder: {}\n'.format(self.AutoEncoder))
    self.log_file.write('Predictor: {}\n'.format(self.predictor))

    self.summary_dir = summary_dir
    self.writer = SummaryWriter(log_dir=os.path.join(self.summary_dir, 'tensorboard'))
    self.global_step = 0
  ...
  def train(self, train_data, test_data, batch_size, learning_rate, num_epochs, num_workers, log_every, print_every,
    ...
    if self.labels_size != 0:
      optimizer = torch.optim.Adam(list(self.AutoEncoder.parameters()) + list(self.predictor.parameters()),
                                   lr=learning_rate)
      print('optimizing both AutoEncoder and predictor')
      self.log_file.write('optimizing both AutoEncoder and predictor\n')
    else:
      optimizer = torch.optim.Adam(self.AutoEncoder.parameters(), lr=learning_rate)
      print('optimizing AutoEncoder')
      self.log_file.write('optimizing AutoEncoder\n')
    ...
    if ckpt:
      ...
      # do not load the optimizer state and epoch
    ...
    for epoch in range(start_epoch, num_epochs):
      ...
      if (epoch + 1) % log_every == 0:
        ...
        # Do not save at each epoch to avoid filling the disk
  ...
  def epoch(self, data_loader, optimizer, epoch, print_every, w_prior, w_pred_loss, l1_beta, l1_beta_c, l1_beta_ci,
    ...
    for iter, data in enumerate(data_loader):
      ...
      bows_input = data.get("M_bow_input", None)
      bows_input = bows_input.reshape(bows_input.shape[0], -1)
      bows_output = data.get("M_bow_output", None)
      bows_output = bows_output.reshape(bows_output.shape[0], -1)

      embeddings_input = data.get("M_embeddings_input", None)
      embeddings_output = data.get("M_embeddings_output", None)
      ...
      if self.decoder_output == 'bow':
        reconstruction_loss = F.cross_entropy(x_recon, bows_output)
      else:
        # use MSE loss for embeddings
        reconstruction_loss = F.mse_loss(x_recon, embeddings_output)
      ...
      if self.w_prior is None:
        mean_length = torch.sum(bows_input) / bows_input.shape[0]
        vocab_size = bows_input.shape[1]
      ...
      if target_labels is not None:
        ...
        accuracy = (predicted_labels == target_labels).sum().item() / target_labels.shape[0]
      else:
        prediction_loss = 0.0
        accuracy = 0.0

      if self.predict_language:
        # predict language based on theta_q, and force the model to not be able to predict the language
        lang_predictions = self.language_predictor(theta_q, None)
        lang_target = lang.squeeze().to(torch.int64)
        lang_predicted = torch.argmax(lang_predictions, dim=1)
        lang_accuracy = (lang_predicted == lang_target).sum().item() / lang_target.shape[0]
        lang_probs = F.softmax(lang_predictions, dim=1)

        # use mmd loss to force the model to not be able to predict the language
        uniform = torch.ones_like(lang_predictions) / lang_predictions.shape[1]
        mmd_loss_lang = compute_mmd_loss(lang_probs, uniform, device=self.device, t=0.1)

        # use kl divergence to force the model to not be able to predict the language
        kl_loss_lang = F.kl_div(F.log_softmax(lang_predictions, dim=1), F.log_softmax(uniform, dim=1), reduction='batchmean', log_target=True)

        mmd_loss_lang_data = mmd_loss_lang.item()
      else:
        mmd_loss_lang = torch.tensor(0.0)
        kl_loss_lang = torch.tensor(0.0)
        mmd_loss_lang_data = torch.tensor(0.0)
        lang_accuracy = torch.tensor(0.0)

      # Total loss
      # w_prior /= 10.0
      loss = (reconstruction_loss + mmd_loss * w_prior + prediction_loss * w_pred_loss
              + decoder_sparsity_loss +kl_loss_lang * self.w_lang)
      # loss = reconstruction_loss
      # loss = prediction_loss * w_pred_loss
      ...
      if (iter + 1) % print_every == 0:
        if validation:
          print(
            f'Epoch {(epoch + 1):>3d}\tIter {(iter + 1):>4d}'
            f'\tValidation Loss:{loss.item():<.7f}\n'
            f'Mean Rec Loss:{reconstruction_loss.item():<.7f}'
            f'\nMMD Loss:{mmd_loss.item():<.7f}'
            f'\nMMD Lang Loss:{mmd_loss_lang_data:<.7f}'
            f'\nKL Lang Loss:{kl_loss_lang.item()}'
            f'\nSparsity Loss:{decoder_sparsity_loss:<.7f}'
            f'\nMean Pred Loss:{prediction_loss:<.7f}'
            f'\nAccuracy:{accuracy:<.7f}'
            f'\nLang Acc:{lang_accuracy:<.7f}\n')
          # flush the output to the console
          sys.stdout.flush()
          # write the same information to a log file
          self.log_file.write('Epoch {}\tIter {}\tValidation Loss:{:.7f}\n'.format(epoch + 1, iter + 1, loss.item()))
          self.log_file.write('Mean Rec Loss:{:.7f}\n'.format(reconstruction_loss.item()))
          self.log_file.write('MMD Loss:{:.7f}\n'.format(mmd_loss.item()))
          self.log_file.write('MMD Lang Loss:{:.7f}\n'.format(mmd_loss_lang_data))
          self.log_file.write('KL Lang Loss:{:.7f}\n'.format(kl_loss_lang.item()))
          self.log_file.write('Sparsity Loss:{:.7f}\n'.format(decoder_sparsity_loss))
          self.log_file.write('Mean Pred Loss:{:.7f}\n'.format(prediction_loss))
          self.log_file.write('Accuracy:{:.7f}\n'.format(accuracy))
          self.log_file.write('Lang Acc:{:.7f}\n'.format(lang_accuracy))
          self.log_file.write('\n')
          self.log_file.flush()
          # also write to tensorboard by iteration
          self.writer.add_scalar('Validation Loss', loss.item(), self.global_step)
          self.writer.add_scalar('Mean Rec Loss', reconstruction_loss.item(), self.global_step)
          self.writer.add_scalar('MMD Loss', mmd_loss.item(), self.global_step)
          self.writer.add_scalar('MMD Lang Loss', mmd_loss_lang_data, self.global_step)
          self.writer.add_scalar('Sparsity Loss', decoder_sparsity_loss, self.global_step)
          self.writer.add_scalar('Mean Pred Loss', prediction_loss, self.global_step)
          self.writer.add_scalar('Accuracy', accuracy, self.global_step)
          self.writer.add_scalar('Lang Acc', lang_accuracy, self.global_step)
          self.writer.flush()
        else:
          print(
            f'Epoch {(epoch + 1):>3d}\tIter {(iter + 1):>4d}'
          ...
          # flush the output to the console
          sys.stdout.flush()
          self.log_file.write('Epoch {}\tIter {}\tTraining Loss:{:.7f}\n'.format(epoch + 1, iter + 1, loss.item()))
          self.log_file.write('Mean Rec Loss:{:.7f}\n'.format(reconstruction_loss.item()))
          self.log_file.write('MMD Loss:{:.7f}\n'.format(mmd_loss.item()))
          self.log_file.write('MMD Lang Loss:{:.7f}\n'.format(mmd_loss_lang_data))
          self.log_file.write('KL Lang Loss:{:.7f}\n'.format(kl_loss_lang.item()))
  ...
  def get_topic_words(self, content_covariates=None, topK=10):
    """
