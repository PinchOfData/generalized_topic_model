...
class AutoEncoderMLP(nn.Module):
    ...
    def __init__(
        ...
        self.encoder2 = nn.ModuleDict({
            f'enc2_{i}': nn.Linear(encoder_dims[i], encoder_dims[i+1], bias = encoder_bias)
            for i in range(len(encoder_dims)-1)
        })
        self.proj = nn.Linear(encoder_dims[-1], n_topics, bias = encoder_bias)

        self.decoder = nn.ModuleDict({
            f'dec_{i}': nn.Linear(decoder_dims[i], decoder_dims[i+1], bias = decoder_bias)
            for i in range(len(decoder_dims)-1)
        })
        # create another decoder for the second language
        self.decoder2 = nn.ModuleDict({
            f'dec2_{i}': nn.Linear(decoder_dims[i], decoder_dims[i+1], bias = decoder_bias)
            for i in range(len(decoder_dims)-1)
        })
    ...
    def encode2(self, x):
        ...
        """
        hid = x
        for i, (_,layer) in enumerate(self.encoder2.items()):
            hid = self.dropout(layer(hid))
            if i < len(self.encoder2)-1 and self.encoder_non_linear_activation is not None:
                hid = self.encoder_nonlin(hid)
        return hid
    ...
    def decode2(self, z):
        ...
        """
        hid = z
        for i, (_, layer) in enumerate(self.decoder2.items()):
            hid = layer(hid)
            if i < len(self.decoder2)-1 and self.decoder_non_linear_activation is not None:
                hid = self.decoder_nonlin(hid)
        return hid
    ...
    def forward(self, x, prevalence_covariates, content_covariates, target_labels, lang, to_simplex=True):
        ...
        z_before = torch.where(lang == 0, z1, z2)
        # project z to labels
        z = self.proj(z_before)
