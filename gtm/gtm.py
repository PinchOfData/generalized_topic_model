...
class GTM:
	...
	def __init__(
		...
		if doc_topic_prior == 'dirichlet':
			self.prior = DirichletPrior(prevalence_covariate_size, n_topics, alpha,
										prevalence_covariates_regularization, tol, device=self.device)
	...
	def train(self, train_data, test_data, batch_size, learning_rate, num_epochs, num_workers, log_every, print_every,
		...
		if self.decoder_type == 'sage':
			...
			if self.content_covariate_size != 0:
				l1_beta_c = 0.5 * np.ones([self.bow_size, self.content_covariate_size], dtype=np.float32) / float(
					n_train)
		...
		for epoch in range(start_epoch, num_epochs):
			...
			if test_data is not None:
				validation_loss = self.epoch(test_data_loader, optimizer, epoch, print_every, w_prior, w_pred_loss,
											 l1_beta, l1_beta_c, l1_beta_ci, validation=True)
		...
		with torch.no_grad():
			...
			for data in data_loader:
				...
				if self.encoder_input == 'bow':
					...
					x_input = embeddings
				_, _, thetas = self.AutoEncoder(x_input, prevalence_covariates, content_covariates, target_labels,
												to_simplex)
	...
	def get_top_docs(self, dataset, topic_id=None, return_df=False, topK=1):
		...
		if return_df is False:
			if topic_id is None:
				...
				for i in top_k_indices_df['Topic_{}'.format(topic_id)]:
					print("Topic: {} | Document index: {} | Topic share: {}".format(topic_id, i, doc_topic_distribution[
						i, topic_id]))
					print(dataset.df['doc'].iloc[i])
					print('\n')
		else:
