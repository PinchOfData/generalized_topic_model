...
class GTMCorpus(Dataset):
  ...
  def __init__(self, df, prevalence=None, content=None, prediction=None, labels=None, embeddings_type=None,
               ...
               doc2vec_args={},
               en2zh='../data/wiki_shorts/similar/en_to_zh_dict.pt',
               zh2en='../data/wiki_shorts/similar/zh_to_en_dict.pt'):
    """
    ...
    self.df = df
    self.en2zh = torch.load(en2zh)
    self.zh2en = torch.load(zh2en)
    ...
    # en is 0, zh is 1, iterate over list
    for i in range(len(self.language)):
      if self.language[i] == 'en':
        self.language[i] = 0
      else:
        self.language[i] = 1
